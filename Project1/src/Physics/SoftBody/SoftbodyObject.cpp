...
void SoftbodyObject::AddSticksForAllPoints()
{
	for (Point* point : listOfPoints)
	{
		for (Point* otherPoint : listOfPoints)
		{
			...
			float length = glm::length(point->position - otherPoint->position);
			

			if ( (length >0) && !CheckStickExists(point, otherPoint))
			{
...
bool SoftbodyObject::CheckStickExists(Point* point, Point* otherPoint)
{
	...
	{
		if (stick->pointA == point && stick->pointB == otherPoint || stick->pointA == otherPoint && stick->pointB == point)
		{
			return true;
		}
	}
...
void SoftbodyObject::ApplyCollision()
{
	...
	for (PhysicsObject* physicsObject : listOfPhysicsObject)
	{
		...
		for (Point* point : listOfPoints)
		{
			...
			case AABB:
				if (CheckSoftBodyAABBCollision(point, physicsObject->UpdateAABB()))
				{
